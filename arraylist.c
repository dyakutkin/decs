...
void ArrayList__append(ArrayList *self, void *elem) {
  ...
  char* current_latest_elem_ptr = (char*)self->elements + self->elem_size * self->len;
  memcpy(current_latest_elem_ptr, elem, self->elem_size);
  self->len++;
}
...
void *ArrayList__get(ArrayList *self, size_t idx) {
  ...
  char* elem_ptr = (char*)self->elements + self->elem_size * idx;
